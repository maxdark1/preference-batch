...
public class SchedulerConfig extends DefaultBatchConfigurer
{
	 ...
	 * The Upload tasklet.
	 */
	private final UploadTasklet uploadTasklet;
	...
	public Job processJob()
	{
		...
		return jobBuilderFactory.get("processJob").incrementer(new RunIdIncrementer()).listener(jobListener).start(orderStep1())
				.on("*").to(orderStep2()).on("*").to(orderStep3()).next(orderStep4()).build().build();
	...
	@Bean
	public Step orderStep3()
	{
		return stepBuilderFactory.get("orderStep3").tasklet(uploadTasklet).transactionManager(transactionManager).build();
	}
