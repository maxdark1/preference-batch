...
public class RegistrationItemProcessor implements ItemProcessor<InboundRegistration, FileInboundStgTable>
{
	...
	public FileInboundStgTable process(InboundRegistration item) throws Exception
	{
		...
		builder.status("NS").src_language_pref(item.getLanguage_Preference().trim().toUpperCase()).updated_date(new Date()).src_date(asOfDate)
				...
				.value20(item.getValue_20()).inserted_by("test_batch").inserted_date(new Date());
