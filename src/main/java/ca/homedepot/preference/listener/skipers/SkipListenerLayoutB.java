...
public class SkipListenerLayoutB extends SkipFileService implements SkipListener<EmailOptOuts, FileInboundStgTable> {
    ...
    public void onSkipInProcess(EmailOptOuts item, Throwable t) {
        ...
        FileInboundStgTable fileInboundStgTable = FileInboundStgTable.builder().file_id(getFromTableFileID(item.getFileName(), jobName)).src_email_address(item.getEmailAddress()).fileName(item.getFileName())
