...
public class OutboundSqlQueriesConstants
{
	...
	public static final String SQL_SELECT_PREFERENCES_FOR_CITI_SUP_STEP_1 = "WITH citisuppresion as\n"
			...
			+ "\t )\n"
			+ "\tSELECT null as First_name, null as middle_initial, null as last_name, null as addr_line_1, null as addr_line_2, null as city, null as state_cd, null as postal_cd, 'N' as dm_opt_out,\n"
			+ "\t email.email as email_addr,\n" + "\t\tnull as phone,\n" + "\t\tnull as sms_mobile_phone,\n"
			...
			+ "\tSELECT null as First_name, null as middle_initial, null as last_name, null as addr_line_1, null as addr_line_2, null as city, null as state_cd, null as postal_cd, null as dm_opt_out,\n"
			+ "\t null as email_addr,null as phone, phone.phone_number as sms_mobile_phone,\n" + "\tnull as business_name,\n"
			+ "\t 'N' as email_opt_out,\n" + "\t'N' as Phone_Opt_Out,\n" + "\tCASE\n"
			...
			+ "\tINNER JOIN hdpc_master_key masterkey on master.key_id = masterkey.key_id AND masterkey.key_value = 'PHONE_TYPE'\n"
			+ "\tWHERE custphone.active\n" + "\tUNION \n" + "\tSELECT first_name, middle_name as middle_initial, last_name, \n"
			+ "\taddress.address_line_1 as addr_line_1, address.address_line_2 as addr_line_2, \n"
			+ "\taddress.city as city, address.province as state_cd, address.postal_code as postal_cd,\n" + "\t \n"
			+ "\tCASE WHEN custAdd.permission_val = true THEN 'N' \n" + "     WHEN custAdd.permission_val = false THEN 'Y' \n"
			+ "\t WHEN custAdd.permission_val is null THEN 'U' END as DM_OPT_OUT,\n" + "\t \n" + "\t null as email_addr,\n"
			+ "\t\tnull as phone,\n" + "\t\tnull as sms_mobile_phone,\n" + "\t extn.org_name as business_name,\n"
			+ "\t\t\t'N' as email_opt_out,\n" + "\t\t'N' as phone_opt_out,\n" + "\t\t'N' as sms_opt_out\n"
	...
	public static final String SQL_SELECT_FOR_FLEX_ATTRIBUTES_INTERNAL_DESTINATION = "select MAX(hfc.file_id) file_id\n"
			...
			+ "\t\t left join hdpc_master_id_rel idrel on c.source_type = idrel.pcam_id\n"
			...
			+ "\t\t left join hdpc_master_id_rel custtype ON custtype.pcam_id::varchar = cx.cust_type\n"
