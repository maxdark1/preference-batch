...
class PreferenceServiceImplTest {
    ...
    void preferencesRegistration() {
        ...
        Mockito.when(requestBodySpec.body(Mono.just(registrationRequests), new ParameterizedTypeReference<List<RegistrationRequest>>() {
    ...
    void insert() {
        ...
        int value = 1;
        Mockito.when(jdbcTemplate.update(anyString(),  eq(job_name), eq(status), eq(start_time),
                eq(inserted_by), eq(inserted_date))).thenReturn(value);
